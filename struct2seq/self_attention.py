...
class LSTMLayer(nn.Module):
    def __init__(self, num_hidden, num_in, args, num_heads=4, dropout=0.1):
        ...
        self.LSTM = nn.LSTM(num_hidden, num_hidden, batch_first = True, bidirectional = args.bidirectional)
    ...
    def forward(self, h_V, h_E, mask_V=None, mask_attend=None):
        ...
        # LSTM
        lstm_hidden = (torch.randn(self.bidirectional, h_V.shape[0], h_V.shape[-1]), torch.randn(self.bidirectional, h_V.shape[0], h_V.shape[-1]))
        dh, lstm_hidden = self.LSTM(h_V, lstm_hidden)
        if self.bidirectional == 2: 
            num_hidden = dh.shape[-1] / 2
            dh = (dh[:, :, :num_hidden] + dh[:, :, num_hidden:])/2
...
class LSTMLayer2(nn.Module):
    def __init__(self, num_hidden, num_in, z_dim, num_heads=4, dropout=0.1):
        ...
        self.dropout = nn.Dropout(dropout)
        self.norm = nn.ModuleList([Normalize(num_hidden) for _ in range(2)])
        self.fc_v = nn.Linear(z_dim, num_hidden)

        self.attention = NeighborAttention2(num_hidden, num_in, num_heads)
        self.LSTM = nn.LSTM(num_hidden, num_hidden, batch_first = True, bidirectional = False)
        self.dense = PositionWiseFeedForward(num_hidden, num_hidden * 4)
        self.bidirectional = 1
    ...
    def forward(self, h_V, h_E, mask_V=None, mask_attend=None):
        ...
        # Self-attention
        n_nodes = h_E.shape[1]
        h_V = self.fc_v(h_V.unsqueeze(1).expand(-1, n_nodes, -1))
...
class MPNNLayer(nn.Module):
    def __init__(self, num_hidden, num_in, args, dropout=0.1, num_heads=None, scale=30):
...
class NeighborAttention(nn.Module):
    ...
    def forward(self, h_V, h_E, mask_attend=None):
        ...
        attend_logits = torch.matmul(Q, K).view([n_batch, n_nodes, n_neighbors, n_heads]).transpose(-2,-1)#[batch,nodes,heads,neighbor]
        attend_logits = attend_logits / np.sqrt(d)
        
        if mask_attend is not None:
            # Masked softmax
            mask = mask_attend.unsqueeze(2).expand(-1,-1,n_heads,-1)  #[batch, nodes, heads, K]
            attend = self._masked_softmax(attend_logits, mask)   #[batch, nodes, heads]
        else:
            attend = F.softmax(attend_logits, -1)  #[n_batch, n_nodes, heads]

        # Attentive reduction
        h_V_update = torch.matmul(attend.unsqueeze(-2), V.transpose(2,3)) 
        # [n_batch, n_node, 1, n_neighbors] @ [n_batch, n_nodes, n_head, neighbor, d] -> [n_batch, n_node, n_heads, d]
        h_V_update = h_V_update.view([n_batch, n_nodes, self.num_hidden]) #[n_batch, n_node, n_hidden]
        h_V_update = self.W_O(h_V_update) # [N_batch, N_nodes, N_hidden]
        return h_V_update
...
class NeighborAttention2(nn.Module):
    def __init__(self, num_hidden, num_in, num_z, num_heads=4):
        super(NeighborAttention2, self).__init__()
        self.num_heads = num_heads
        self.num_hidden = num_hidden
    ...
    def forward(self, h_V, h_E, mask_attend=None):
        """ Self-attention, graph-structured O(Nk)
        Args:
            h_V:            Node features           [N_batch, z_dim(32)]
            h_E:            Neighbor features       [N_batch, N_nodes, K, N_hidden]
            mask_attend:    Mask for attention      [N_batch, N_nodes, K]
        Returns:
            h_V:            Node update             [N_batch, N_nodes, N_hidden]
        """
