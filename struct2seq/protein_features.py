...
class ProteinFeatures(nn.Module):
    def __init__(self, edge_features, node_features, args, num_positional_embeddings=16,
    ...
    def _rbf(self, D):
        ...
        D_mu = D_mu.view([1,1,1,-1]).to(self.device)
    ...
    def _orientations_coarse(self, X, E_idx, eps=1e-6):
        ...
        AD_features = F.pad(AD_features, (0,0,1,2), 'constant', 0)  ##연속적인 Ca polar coordinate
    ...
    def _dihedrals(self, X, eps=1e-7):
        ...
        dX = X[:,:-1,:] - X[:,1:,:]
        U = F.normalize(dX, dim=-1)
        u_0 = -U[:,:-2,:]  # ab psi: N->CA->C->Nn
        u_1 = U[:,1:-1,:]  # cb omega: CAp->C->N->Ca
        u_2 = U[:,2:,:]    # dc phi: Cp->N->CA->C
        
        # Backbone normals
        n_1 = F.normalize(torch.cross(u_0, u_1), dim=-1) #u
        n_2 = F.normalize(torch.cross(u_2, u_1), dim=-1) #v
        ...
        D[:, :, 2] = torch.cat((D[:, -1, 2].view(-1 ,1), D[:, :-1, 2]), dim = -1)
